...
(function (window, document) {
  ...
  function readDone(scope) {
    var read = getRead(), total = 0;
    manifestModules().forEach(function (md) {
      if (scope !== 'curso' && scope !== 'modulo:' + md.id && scope !== 'trilha:' + md.track) return;
      for (var n = 1; n <= md.topics; n++) if (read['modulo-' + md.id + '#topico-' + n] === true) total++;
    });
    return total;
  }
  ...
  function renderHighlights(container) {
    ...
    for (var blockId in notes) {
      ...
      var blockModule = blockId.match(/^m(\d+-\d+)-/);
      if (!block && blockModule && !document.querySelector('[data-inema-module="' + blockModule[1] + '"]')) continue;
  ...
  function jumpTo(item) {
    ...
    if (!target) {
      if (item.kind === 'topic') navigateTopic(item.id);
      else { var match = String(item.blockId || '').match(/^m(\d+-\d+)-t(\d+)-/); if (match) navigateTopic('modulo-' + match[1] + '#topico-' + match[2]); }
    }
  ...
  function navigateTopic(anchor) {
    var parts = String(anchor || '').match(/^modulo-(\d+-\d+)#(topico-\d+)$/);
    if (!parts) return false;
    var md = manifestModules().filter(function (m) { return m.id === parts[1]; })[0];
    if (!md) return false;
    var root = document.querySelector('meta[name="inema-root"]');
    var url = new URL(md.href, new URL(root ? root.content : './', window.location.href));
    url.hash = parts[2];
  ...
  function resume() {
    var anchor = getMeta().lastTopicAnchor;
    if (!anchor) return false;
    var target = document.querySelector('[data-inema-topic="' + cssAttrEsc(anchor) + '"]');
    if (target) { openAncestors(target); target.scrollIntoView({block:'start', behavior:'auto'}); return true; }
    return navigateTopic(anchor);
  }
  ...
  function importJSON(text, opts) {
    ...
    var record = function(v) { return v && typeof v === 'object' && !Array.isArray(v); };
    var dangerous = function(v) {
      if (!v || typeof v !== 'object') return false;
      return Object.keys(v).some(function(k) { return ['__proto__','prototype','constructor'].indexOf(k) !== -1 || dangerous(v[k]); });
    };
    if (parsed.schemaVersion !== SCHEMA_VERSION || parsed.courseId !== S.courseId || dangerous(parsed)) {
      return {ok:false, applied:0, skipped:0, errors:["Archivo de otro curso o versión no válida."]};
    }
    if (['read','doubts','notes','checks','meta'].some(function(k) { return parsed[k] != null && !record(parsed[k]); })) {
      return {ok:false, applied:0, skipped:0, errors:["Estructura de progreso no válida."]};
    }
    if (Object.keys(parsed.notes || {}).some(function(k) { return !Array.isArray(parsed.notes[k]) || parsed.notes[k].some(function(n) { return !record(n) || typeof n.id !== 'string' || typeof n.quote !== 'string'; }); })) {
      return {ok:false, applied:0, skipped:0, errors:["Notas no válidas."]};
    }
    var incoming = migrate(parsed);
    var extra = Object.assign({}, storageGet(key('extra'), {}) || {});
    Object.keys(parsed).forEach(function(k) { if (['schemaVersion','courseId','exportedAt','read','doubts','notes','checks','meta'].indexOf(k) === -1) extra[k] = parsed[k]; });
  ...
  function bindGlobalListeners() {
    ...
    // Delegação também cobre a barra de aprendizagem fora do main.
